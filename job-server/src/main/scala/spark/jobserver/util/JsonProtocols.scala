...
object JsonProtocols extends DefaultJsonProtocol {
  ...
  val DATE_PATTERN = "yyyy-MM-dd HH-mm-ss SSS Z"
  ...
  implicit object JobInfoJsonFormat extends RootJsonFormat[JobInfo] {
    ...
    def write(i: JobInfo): JsObject =
      JsObject(
        ...
        "startTime" -> JsString(df.format(i.startTime)),
  ...
  private def fromDateTime(dt: ZonedDateTime): String = df.format(dt)
  private def toDateTime(s: String): ZonedDateTime = ZonedDateTime.parse(s, df)
