...
class MetaDataZookeeperDAOSpec extends FunSpec with TestJarFinder with FunSpecLike
  ...
  // Binaries
  val binJar = BinaryInfo("binaryWithJar", BinaryType.Jar, ZonedDateTime.now(),
      Some(BinaryDAO.calculateBinaryHashString("1".getBytes)))
  val binEgg = BinaryInfo("binaryWithEgg", BinaryType.Egg, ZonedDateTime.now(),
      Some(BinaryDAO.calculateBinaryHashString("2".getBytes)))
  val binWheel = BinaryInfo("binaryWithWheel", BinaryType.Wheel, ZonedDateTime.now(),
      Some(BinaryDAO.calculateBinaryHashString("3".getBytes)))
  val binURI = BinaryInfo("http://foo/bar", BinaryType.URI, ZonedDateTime.now(),
      Some(BinaryDAO.calculateBinaryHashString("42".getBytes)))
  val binJarV2 = BinaryInfo("binaryWithJar", BinaryType.Jar, ZonedDateTime.now().plusHours(1),
      Some(BinaryDAO.calculateBinaryHashString("3".getBytes)))
  val binElse = BinaryInfo("anotherBinaryWithJar", BinaryType.Jar, ZonedDateTime.now(),
      Some(BinaryDAO.calculateBinaryHashString("3".getBytes)))
  ...
  val normalJob = JobInfo("someJobId", "someContextId", "someContextName",
      "someClassPath", "someState", ZonedDateTime.now(), Some(ZonedDateTime.now()),
  ...
  val minimalJob = JobInfo("someOtherJobId", "someContextId", "someOtherContextName",
      "someClassPath", "someState", ZonedDateTime.now().plusHours(1), None, None, Seq(binJar))
  val sameIdJob = JobInfo("someJobId", "someOtherContextId", "thirdContextName",
      "someClassPath", "someState", ZonedDateTime.now().minusHours(1), None, None, Seq(binJar))
  val anotherJob = JobInfo("thirdJobId", "someOtherContextId", "thirdContextName",
      "someClassPath", "anotherState", ZonedDateTime.now().minusHours(1), None, None, Seq(binJar))
  val multiJarJob = JobInfo("multiJarJobId", "someOtherContextId", "thirdContextName",
    "someClassPath", "anotherState", ZonedDateTime.now().minusHours(1), None, None,
  ...
  describe("Binary tests") {
    ...
    it("should list all binaries with given storage id with unique (name, binaryStorageId)") {
      Await.result(dao.saveBinary(binJar.appName, binJar.binaryType,
        ZonedDateTime.now(), binJar.binaryStorageId.get), timeout)
      Await.result(dao.saveBinary(binJar.appName, binJar.binaryType,
        binJar.uploadTime, binJar.binaryStorageId.get), timeout)
